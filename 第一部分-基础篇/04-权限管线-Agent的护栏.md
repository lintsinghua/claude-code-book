...
│       ↓                                                              │
│  ┌────────────────────┐                                              │
│  │ 阶段一：validateInput│──── 失败 → ask（优雅降级为用户确认）          │
│  │ Zod Schema 验证     │                                              │
│  └────────┬───────────┘                                              │
│           ↓ 通过                                                     │
│  ┌────────────────────┐                                              │
│  │ 阶段二：规则匹配     │──── deny → 拒绝（不可覆盖）                  │
│  │ deny > ask > allow  │──── ask  → 进入阶段四                        │
│  │ 优先级铁律          │──── allow → 放行                              │
│  └────────┬───────────┘                                              │
│           ↓ 无匹配规则                                               │
│  ┌────────────────────┐                                              │
│  │ 阶段三：checkPerms  │──── deny → 拒绝                              │
│  │ 工具特定的上下文评估 │──── ask  → 进入阶段四                        │
│  │                      │──── allow → 放行                            │
│  │                      │──── passthrough → 进入阶段四                 │
│  └────────┬───────────┘                                              │
│           ↓ ask 或 passthrough                                       │
│  ┌────────────────────┐                                              │
│  │ 阶段四：交互式提示   │──── hook 决策 → allow/deny                   │
│  │ Hook → 分类器 → 用户│──── classifier → allow/deny                  │
│  │ 多决策者竞争         │──── user → allow/deny/allow-once             │
│  └────────────────────┘                                              │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
```
...
│  │ 逐次确认 │  │ 只读为主 │  │AI分类器  │  │   完全跳过       │    │
│  │          │  │          │  │自动审批  │  │                  │    │
│  │ 每次工具 │  │ 写入工具 │  │ 分类器代 │  │ 除 deny 规则和   │    │
│  │ 调用都需 │  │ 被 deny  │  │ 替人工审 │  │ safetyCheck 外   │    │
│  │ 用户确认 │  │ 只读放行 │  │ 批       │  │ 全部自动放行     │    │
│  └──────────┘  └──────────┘  └──────────┘  └──────────────────┘    │
│                                                                      │
│  ┌──────────┐                                                       │
│  │ bubble   │  内部模式，不对外暴露                                   │
│  │ 子智能体 │  子智能体权限"冒泡"回主智能体                           │
│  │ 权限冒泡 │                                                        │
│  └──────────┘                                                       │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│                                                                     │
│  精确匹配          前缀匹配           通配符匹配                      │
│  Bash(npm test)    Bash(npm:*)        Bash(git commit *)            │
│                                                                     │
│  精确匹配一条      匹配所有以 npm      匹配 git commit                │
│  特定命令          开头的命令          后跟任意参数                   │
│                                                                     │
│  最安全/最窄       中等/适中          最灵活/最宽                     │
│  零误放行风险      可能误放行          需要仔细设计模式                │
│                                                                     │
│  适用：CI/CD       适用：工具链       适用：版本控制等                 │
│  中的固定步骤      前缀命令           命令族                          │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
