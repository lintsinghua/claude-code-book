...
```
┌─────────────────────────────────────────────────────┐
│               BaseAgentDefinition 字段分类            │
├──────────────┬──────────────────────────────────────┤
│  身份与描述   │  agentType, whenToUse, color          │
├──────────────┼──────────────────────────────────────┤
│  工具与能力   │  allowedTools, disallowedTools,       │
│              │  skills, mcpServers                   │
├──────────────┼──────────────────────────────────────┤
│  执行控制     │  model, effort, maxTurns,            │
│              │  permissionMode, background           │
├──────────────┼──────────────────────────────────────┤
│  上下文管理   │  omitClaudeMd, isolationMode,        │
│              │  hooks, memory                        │
└──────────────┴──────────────────────────────────────┘
```
...
```
┌──────────────────────────────────────────────────┐
│          Verification Agent 的工作模式             │
│                                                   │
│   主智能体完成了代码修改                             │
│        │                                          │
│        ▼                                          │
│   Verification Agent 被激活（后台）                 │
│        │                                          │
│        ├─→ 尝试运行测试，寻找失败的用例              │
│        ├─→ 检查边界条件和异常路径                    │
│        ├─→ 寻找遗漏的 import 或类型错误             │
...
```
┌─────────────────────────────────────────────────────────┐
│            CacheSafeParams 五维缓存契约                    │
│                                                          │
│  ┌──────────────┐                                        │
│  │  systemPrompt │ ← 系统提示必须字节一致                   │
│  └──────┬───────┘                                        │
│         │                                                │
│  ┌──────▼───────┐                                        │
...
│         │                                                │
│  ┌──────▼──────────┐                                     │
│  │ systemContext    │ ← 系统注入的上下文信息                 │
│  └──────┬──────────┘                                     │
│         │                                                │
│  ┌──────▼──────────┐                                     │
│  │ toolUseContext   │ ← 工具定义 + 模型选择                  │
│  │ (tools + model)  │                                    │
│  └──────┬──────────┘                                     │
│         │                                                │
│  ┌──────▼──────────────┐                                 │
│  │ forkContextMessages  │ ← 对话历史前缀消息                 │
│  └─────────────────────┘                                 │
│                                                          │
│  ★ 五个维度全部一致 → 缓存命中 → 节省 token 和延迟          │
│  ✗ 任一维度偏离 → 缓存失效 → 完整重新处理                    │
└─────────────────────────────────────────────────────────┘
```
...
│  │ WebSearch, WebFetch + MCP 工具               │     │
│  └─────────────────────────────────────────────┘     │
│                    │                                  │
│  第三层：filterToolsForAgent（最终仲裁）                 │
│  ┌─────────────────────────────────────────────┐     │
│  │ MCP 工具 → 始终可用                           │     │
│  │ 全局禁止 → 一律排除                            │     │
│  │ 异步智能体 → 白名单过滤                        │     │
...
│                                                              │
│   ┌──────────┐                                               │
│   │ Created  │ Agent 工具被调用，智能体定义被解析               │
│   └────┬─────┘                                               │
│        │                                                     │
│        ▼                                                     │
│   ┌──────────┐                                               │
│   │Context   │ 构建系统提示、工具池、MCP 连接                  │
│   │Built     │                                               │
│   └────┬─────┘                                               │
│        │                                                     │
│        ▼                                                     │
│   ┌──────────┐     ┌──────────┐                             │
│   │ Running  │◄───►│ Waiting  │ 等待工具调用结果               │
│   └────┬─────┘     │ for Tool │                             │
│        │           │  Result  │                             │
│        │           └──────────┘                             │
│        │                                                     │
