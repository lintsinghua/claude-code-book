...
KAIROS（主标志）
  |
  +-- KAIROS_BRIEF      （精简子集）
  +-- KAIROS_CHANNELS   （频道通信，可独立启用）
  +-- KAIROS_DREAM      （Dream 技能集）
  +-- KAIROS_GITHUB_WEBHOOKS  （GitHub 集成）
...
```
FORK_SUBAGENT
  +-- BUILTIN_EXPLORE_PLAN_AGENTS  （需要 fork 能力）

AGENT_TRIGGERS
  +-- AGENT_TRIGGERS_REMOTE        （需要本地触发器基础）
```
...
技能系统（基础）
  |
  +-- MCP_SKILLS                 （从 MCP 发现技能）
  +-- EXPERIMENTAL_SKILL_SEARCH  （语义技能搜索）
  +-- SKILL_IMPROVEMENT          （技能自动优化）
  +-- RUN_SKILL_GENERATOR        （动态生成技能）
  +-- BUILDING_CLAUDE_APPS       （Claude Apps 专用技能）
  +-- REVIEW_ARTIFACT            （审阅专用技能）
```
