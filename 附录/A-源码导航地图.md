...
[9] turn 结束
     |  - 输出最终文本响应
     |  - 检查是否需要 auto-compact（参见"上下文压缩触发路径"）
     |  - 更新 token 使用量统计
     v
[10] 等待下一轮用户输入
```
...
工具调用请求
     ...
     v
[1] 输入合法性校验
     |  - Zod schema 验证：确保输入参数符合工具定义的类型约束
     |  - 若验证失败，直接返回错误（不进入后续流程）
     v
[2] 工具级权限逻辑检查
     |  - 调用工具自身的 checkPermissions() 方法
     |  - 某些工具（如 BashTool）会根据命令内容动态判定
     |  - 返回 allow / deny / ask 之一
     v
[3] PreToolUse 钩子          -- 用户自定义钩子
     |  - 执行用户在 settings.json 中配置的 pre-tool-use 钩子
     |  - 钩子可以返回：继续执行 / 修改参数 / 阻止执行
     |  - 参见第 8 章钩子系统的详细说明
     v
[4] 通用权限系统检查
     |  - 检查 alwaysAllow / alwaysDeny / alwaysAsk 规则
     |  - 检查 permission mode（default / plan / auto）
     |  - 权限规则的优先级：alwaysDeny > alwaysAllow > alwaysAsk
     |  - 在 plan 模式下，仅允许只读工具执行
     |  - 参见第 4 章权限管线的分层设计
     v
[5] 分类器辅助决策（若启用）
     |  - Transcript Classifier：基于对话内容推断权限模式
     |  - Bash Classifier：对 Bash 命令进行安全性分类
     |  - 高置信度的安全操作可自动放行
     v
[执行 / 拒绝]
     |
     v
[6] PostToolUse 钩子         -- 执行后钩子
     |  - 执行用户配置的 post-tool-use 钩子
     |  - 可用于日志记录、结果验证、副作用检查等
     |  - 钩子的返回值不影响已完成的工具执行
```
...
对话主循环 turn 结束
     ...
     v
[1] 检查 token 使用量
     |  - 统计当前 messages[] 数组的总 token 数
     |  - 与压缩阈值（通常为 context window 的 80%-90%）比较
     |  - 若未接近阈值，跳过压缩，直接进入下一个 turn
     v
[2] 触发压缩策略选择
     |  - micro-compact：轻量级压缩，快速缩减，保留关键结构
     |  - auto-compact（标准）：完整压缩，调用 API 生成高质量摘要
     |  - history snip：对已处理历史进行智能裁剪
     |  - context collapse：最激进的折叠策略（实验性）
     v
[3] 执行压缩
     |  - 分组历史消息为压缩单元
     |  - 调用 API 对每组消息生成摘要
     |  - 维护 Prompt Cache 边界（若启用 CACHED_MICROCOMPACT）
     |  - 替换原始消息为摘要消息
     v
[4] 压缩后处理
     |  - 验证压缩后的 token 使用量在安全范围内
     |  - 更新 messages[] 数组
     |  - 触发压缩提醒（若启用 COMPACTION_REMINDERS）
     |  - 记录压缩前后对比到日志
     v
[5] 恢复对话主循环
     - 压缩后的 messages[] 数组作为后续 turn 的输入
     - 用户无感知，对话自然继续
```
...
[1] 会话启动 / turn 开始
     ...
     v
[2] 记忆发现
     |  - 从当前目录向上逐级查找 CLAUDE.md 文件
     |  - 加载全局用户记忆（~/.claude/CLAUDE.md）
     |  - 加载项目级记忆（项目根目录/CLAUDE.md）
     |  - 加载嵌套记忆（子目录中的 CLAUDE.md）
     |  - 加载团队记忆（若启用 TEAMMEM）
     v
[3] 相关性匹配
     |  - 对发现的所有记忆片段进行相关性评分
...
[4] 记忆注入
     |  - 将选中的记忆内容注入到系统提示中
     |  - 按优先级排序，高优先级记忆放在前面
     |  - 超出 token 预算的记忆片段被截断或跳过
     v
[5] 记忆生效
     - 注入的记忆内容在当前 turn 的模型推理中生效
     - 模型可以参考这些记忆做出更符合用户偏好的决策
```
...
[3] 工具注册
     ...
     v
[4] 运行时调用
     |  - 模型返回 tool_use 时，MCP 工具与内置工具走相同的路由流程
     |  - MCP 工具的执行通过协议通道转发到对应的 MCP 服务器
     |  - 执行结果通过协议通道返回并回填到消息数组
     v
[5] 连接生命周期管理
     - 监听连接状态，断线自动重连
     - 会话结束时清理连接资源
...
```
[1] 父智能体决定 fork
     |  - 模型在推理过程中决定需要创建子智能体处理子任务
     |  - 通过 AgentTool 触发 fork 操作
     v
[2] Fork 上下文创建
     |  - 复制当前消息历史作为子智能体的初始上下文
     |  - 继承权限规则和设置配置
     |  - 子智能体获得独立的 messages 数组
     v
[3] 子智能体独立执行
     |  - 递归调用对话主循环
     |  - 在独立的上下文中执行工具调用
     |  - 支持 fork 内的嵌套 fork（受深度限制约束）
     v
[4] 结果回收
     |  - 子智能体的执行结果回填到父智能体的消息数组
     |  - 父智能体基于子智能体的结果继续推理
     |  - Fork 创建的临时资源被清理
     v
[5] 恢复父智能体执行
     - 父智能体从 fork 点继续执行
     - 子智能体的上下文不再占用 token 预算
```
