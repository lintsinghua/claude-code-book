...
│  0%        85%              90%            95%         100%  │
│                                                             │
│  正常工作    WARNING        AUTOCOMPACT    BLOCKING           │
│  无需干预   提醒用户注意    自动压缩触发   拒绝新请求          │
└─────────────────────────────────────────────────────────────┘
```
...
断路器状态机：
  ...
  [CLOSED] ──── 压缩成功 ────→ [CLOSED]      (正常工作，计数器=0)
     │
     │ 压缩失败
     ▼
  [CLOSED] ──── 连续失败 < 3 ──→ [HALF-OPEN]  (计数器递增，继续尝试)
     │
     │ 连续失败 >= 3
     ▼
  [OPEN] ──── 跳过压缩 ────→ [OPEN]          (熔断，不再尝试)
...
AutoCompact 完整流程：
  ...
  │     ↓  允许用户自定义逻辑介入压缩过程                    │
  │  2. 构建压缩提示                                      │
  │     ↓  选择合适模板（BASE / PARTIAL / PARTIAL_UP_TO）  │
  │  3. 流式生成摘要                                      │
  │     ↓  通过 forked agent 或主线程调用 API               │
  │  4. 处理 prompt-too-long 重试                          │
  │     ↓  截断最老的 API 轮次组并重试（最多 3 次）          │
  │  5. 重建上下文                                        │
  │     ↓  CompactBoundaryMessage + 摘要 + 附件 + 钩子结果 │
  │  6. 执行 PostCompact 钩子                              │
  │     ↓  压缩完成通知                                    │
  │  输出：CompactionResult                                │
  └──────────────────────────────────────────────────────┘
...
三种压缩模板的适用场景：
  ...
  │  └─ 适用：常规自动压缩                                   │
  │                                                         │
  │  PARTIAL_COMPACT_PROMPT (from 方向)                     │
  │  ├─ 场景：仅摘要最近的消息                               │
  │  ├─ 范围：从指定消息开始到当前                            │
  │  └─ 适用：对话前半段已被压缩过，只需处理增量              │
  │                                                         │
  │  PARTIAL_COMPACT_UP_TO_PROMPT (up_to 方向)              │
  │  ├─ 场景：摘要指定消息之前的上下文                        │
  │  ├─ 范围：从对话开始到指定消息                            │
  │  └─ 适用：保留最近的完整消息，压缩早期背景                │
  │                                                         │
...
```
被动压缩（不推荐）：
  用户 ──→ Agent ──→ ... ──→ 95% ──→ 自动压缩 ──→ 摘要（可能丢失重要细节）

主动压缩（推荐）：
  用户 ──→ Agent ──→ 70% ──→ 用户输入 /compact ──→ 摘要（保留用户指定重点）
```
