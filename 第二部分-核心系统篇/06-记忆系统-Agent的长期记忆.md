...
```
                 ...
                 (关于人)                (关于事)
─────────────────────────────────────────────────────
主观/指导性    │  user (用户画像)      │  feedback (反馈指导)
              │  谁在使用？           │  什么做法被验证过？
─────────────────────────────────────────────────────
客观/事实性    │  (通常不需要)         │  project (项目状态)
              │                       │  为什么这样做？
              │                       │  reference (外部引用)
              │                       │  在哪里找更多信息？
─────────────────────────────────────────────────────
...
T1: 主对话开始
T2: 主对话中间，用户说"记住这个偏好"
T3: 主 Agent 主动保存了一条 feedback 记忆 ←── 记忆写入标志被设置
T4: 主对话结束，触发后台提取
T5: 后台 Agent 检测到记忆写入标志，跳过提取 ←── 互斥生效
...
T2: 主对话结束，无主动记忆保存
T3: 触发后台提取
T4: 后台 Agent 分析对话，提取 2 条记忆 ←── 正常提取
```
...
```
方案 A（不同的工具列表）：
  主 Agent 工具集: [Read, Write, Edit, Bash, Grep, Glob, ...]
  提取 Agent 工具集: [Read, Grep, Glob, MemoryWrite]
  → 缓存 key 不同 → 缓存不命中 → 成本增加

方案 B（相同的工具列表，不同的权限）：
  主 Agent 工具集: [Read, Write, Edit, Bash, Grep, Glob, ...]
  提取 Agent 工具集: [Read, Write, Edit, Bash, Grep, Glob, ...]  ← 完全相同
  提取 Agent 权限: canUseTool 回调过滤                    ← 运行时行为不同
  → 缓存 key 相同 → 缓存命中 → 成本节约
```
...
Level 0: 完全不信任 —— 重新获取所有信息
Level 1: 作为线索信任 —— 记忆指出方向，独立验证事实
Level 2: 作为事实信任 —— 记忆就是当前事实（Claude Code 不采用此层次）
```
